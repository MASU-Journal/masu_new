...
if(!empty($_POST['cheif-author-upload'])){
    $reviwer_id = $_SESSION['admin_id'];
    $journal_id = $_POST['journal-id'];
    $redirect_url = 'editorial/c_author_upload.php?manuscript='.$journal_id;
    $pdf_directory = APP_PATH."store_file".DIRECTORY_SEPARATOR."cheif_author_upload".DIRECTORY_SEPARATOR;
    ...
    if(isset($_FILES['reviwer-ceditor-file'])) {
       ...
       /* $insert_data = array();
        ...
        $update_data = array();
        $update_data['chief_author_file'] =  $db->realEscapeString($main_filename);
        $where = array();
        $where['id'] = $journal_id;
