...
$journal_data = $db->query("SELECT id, title, author, affiliation,reviewer_author_file, subject, type, year, status, manuscript_id, resubmitted FROM tbl_journal WHERE status='6'");
...
if(!empty($journal_data->rows)){
  foreach($journal_data->rows as $index => $data){
    $assigned_qry = $db->query("SELECT ta.admin_name,tja.journal_id FROM tbl_journal_assigned AS tja INNER JOIN tbl_admin AS ta ON (ta.admin_id = tja.assigned_to) WHERE tja.journal_id = '".$data->id."' AND tja.status='2' ORDER BY tja.id DESC LIMIT 1");
