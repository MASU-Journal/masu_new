...
$status = '';
$man_cond = '';
$status_cond = " AND tj.status > '-1'";
if(!empty($_GET['ref'])){
  $manuscript_id = trim($_GET['ref']);
  $man_cond = " AND tj.manuscript_id = '$manuscript_id'";
}
...
if(!empty($_GET['article_position'])){
  ...
  $status_cond = " AND tj.status = '$status'";
}

$journal_data = $db->query("SELECT tj.id, tj.title, tj.author, tj.affiliation, tj.subject, 
tj.type, tj.year, tj.status, tj.manuscript_id, tj.resubmitted, ta.admin_name
FROM tbl_journal AS tj
LEFT JOIN tbl_journal_assigned AS tja ON (tj.id = tja.journal_id)
LEFt JOIN tbl_admin AS ta ON (ta.admin_id = tja.assigned_to)
WHERE 1=1 $man_cond $status_cond AND tj.status!='0' order by tj.publish_date DESC");
if(!empty($journal_data->rows)){
  foreach($journal_data->rows as $index => $data){
    if(empty($data->admin_name)) continue;
    $reviewers[$data->id][] = $data->admin_name;
  }
}
...
if(!empty($journal_data->rows)){
  foreach($journal_data->rows as $index => $data){
    $assigned_qry = $db->query("SELECT ta.admin_name,trr.journal_id FROM tbl_review_requests AS trr INNER JOIN tbl_admin AS ta ON (ta.admin_id = trr.reviewer_id) WHERE trr.journal_id = '".$data->id."' AND trr.status='0' ORDER BY trr.id DESC LIMIT 1");
      ...
      ?>
            <div class="card mb-30" style="display: block;padding: 0px;">
                ...
                <div class="card-body" style="padding: 10px 25px;">
                      ...
                      <?php
                      if(!empty($reviewers[$data->id])){ ?>
                      <div class="row">
                        <div class="col-md-2">
                          <span class="text-justify no-margin">
                            <span class="sub-title">Reviewers : </span>
                          </span>
                        </div>
                        <div class="col-md-10">
                          <span class="text-justify no-margin">
                              <?php echo implode(" , ",$reviewers[$data->id]); ?>
                          </span>
                        </div>
                      </div>
                      <?php } ?>
                      ...
                      <div class="row">
                        ...
                        <div class="col-md-10 text-right">
                            <span class="text-right">                            
                              <a href="c-view-journal.php?manuscript=<?php echo $data->id; ?>">
