...
tj.type, tj.year, tj.status, tj.manuscript_id, tj.resubmitted
FROM tbl_journal AS tj WHERE tj.status = '1' AND tj.resubmitted > 0");

//-- INNER JOIN tbl_journal_assigned AS tja ON (tj.id = tja.journal_id)

$autor_data = $db->query("SELECT co.first_name, co.last_name,tj.id FROM tbl_journal tj INNER JOIN tbl_co_author co ON (tj.id = co.journal_id) WHERE tj.status = '1' AND tj.resubmitted > 0");
$authors = array();
...
if(!empty($journal_data->rows)){
  foreach($journal_data->rows as $index => $data){
    $assigned_qry = $db->query("SELECT ta.admin_name,tja.journal_id,tja.assigned_date FROM tbl_journal_assigned AS tja INNER JOIN tbl_admin AS ta ON (ta.admin_id = tja.assigned_to) WHERE tja.journal_id = '".$data->id."' AND tja.status='2' ORDER BY tja.id DESC LIMIT 1");
    if(!empty($assigned_qry->count)){
      foreach($assigned_qry->rows as $k => $rw){
        $assigned_data[$rw->journal_id]['name'] = $rw->admin_name;
         $assigned_data[$rw->journal_id]['date'] = date("d-m-Y", strtotime($rw->assigned_date));
      }
    }
  }
}
            ...
            <div class="card mb-30" style="display: block;padding: 0px;">
                ...
                <div class="card-body" style="padding: 10px 25px;">
                    <div class="row journal-info">
                      ...
                      <?php if(!empty($assigned_data[$data->id]['name'])){ ?>
                      <div class="row">
                      <div class="col-md-2">
                        <span class="text-justify no-margin">
                          <span class="sub-title">Revised By : </span>
                        </span>
                      </div>
                      <div class="col-md-10">
                          <span class="text-justify no-margin">
                              <?php                              
                                echo $assigned_data[$data->id]['name'];
                              ?>
                          </span>
                        </div>
                      </div>
                    <?php } ?>
                    <?php if(!empty($assigned_data[$data->id]['date'])){ ?>
                      <div class="row">
                      <div class="col-md-2">
                        <span class="text-justify no-margin">
                      ...
                      <div class="col-md-10">
                          <span class="text-justify no-margin">
                              <?php                              
                                echo $assigned_data[$data->id]['date'];
                              ?>
                          </span>
                        </div>
                      </div>
