...
$journal_data = $db->query("SELECT id, title, author, affiliation, subject, type, year, status, manuscript_id, resubmitted FROM tbl_journal WHERE status='5'");
$autor_data = $db->query("SELECT co.first_name, co.last_name,tj.id FROM tbl_journal tj INNER JOIN tbl_co_author co ON (tj.id = co.journal_id) WHERE tj.status = '5'");
$authors = array();
$assigned_data = array();
...
if(!empty($journal_data->rows)){
  foreach($journal_data->rows as $index => $data){
    $assigned_qry = $db->query("SELECT ta.admin_name,tja.journal_id FROM tbl_journal_assigned AS tja INNER JOIN tbl_admin AS ta ON (ta.admin_id = tja.assigned_to) WHERE tja.journal_id = '".$data->id."'");
      ...
      margin : 20px 30px;
            ...
            <div class="breadcrumb-area">
                ...
                <ol class="breadcrumb">
                    ...
                    <li class="item">Pending Approval from Editors</li>
                </ol>
            </div>
                ...
                <div class="card-header d-flex justify-content-between align-items-center journal-title" style="margin-top : 15px; margin-bottom: 0px;padding : 5px 25px;">
                    <div class="row">
                      ...
                      <div class="col-md-10 text-right">
                          ...
                          <button style="color:#FFF"  class="btn btn-warning reminderMail"  journal_id="<?php echo $data->manuscript_id;?>">Reminder Mails</button>
                          <a href="c-reassign-journal.php?manuscript=<?php echo $data->id; ?>"><button class="btn btn-primary">Re-assign Editor</button></a>
