<?php
include_once('press-header.php');
$journal_data = $db->query("SELECT id, user_id, title, author, affiliation, subject, type, year, main_file, title_file, info_file, status, manuscript_id, resubmitted,main_file_type,title_file_type FROM tbl_journal WHERE status='10'");
...
$autor_data = $db->query("SELECT co.first_name, co.last_name,tj.id FROM tbl_journal tj INNER JOIN tbl_co_author co ON (tj.id = co.journal_id) WHERE tj.status = '10'");
$authors = array();
                ...
                <div class="card-header d-flex justify-content-between align-items-center journal-title" style="margin-top : 15px; margin-bottom: 0px;padding : 5px 25px;">
                    <div class="row">
                      ...
                      <div class="col-md-10 text-right">
                          ...
                          <a class="btn btn-secondary" href="<?php echo $title_file;?>" target="_blank">Title Copy </a>
                              <a class="btn btn-secondary" href="<?php echo $main_file;?>" target="_blank">Blinded Copy </a>
                          </span>
                        </div>
                      </div>
                </div>
