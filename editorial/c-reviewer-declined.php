...
$journal_data = $db->query("SELECT id, title, author, affiliation, subject, type, year, status, manuscript_id, resubmitted FROM tbl_journal WHERE status='11' AND resubmitted = 0");
...
if(!empty($journal_data->rows)){
  foreach($journal_data->rows as $index => $data){
    $assigned_qry = $db->query("SELECT ta.admin_name,trr.journal_id,trr.updated_at FROM tbl_review_requests AS trr INNER JOIN tbl_admin AS ta ON (ta.admin_id = trr.reviewer_id) WHERE trr.journal_id = '".$data->id."' AND trr.status='2' ORDER BY trr.id DESC LIMIT 1");
    if(!empty($assigned_qry->count)){
      foreach($assigned_qry->rows as $k => $rw){
        $assigned_data[$rw->journal_id][] = $rw->admin_name;
        $assigned_dates[$rw->journal_id][] = date("d-m-Y", strtotime($rw->updated_at));
      }
    } else {
        $assigned_qry = $db->query("SELECT ta.admin_name,tja.journal_id,tdr.reason,tja.modified_date FROM tbl_journal_assigned AS tja 
        ...
        if(!empty($assigned_qry->count)){
          foreach($assigned_qry->rows as $k => $rw){
            $assigned_data[$rw->journal_id][] = $rw->admin_name;
            if(!empty($rw->modified_date)){
              $assigned_dates[$rw->journal_id][] = date("d-m-Y", strtotime($rw->modified_date));
            }            
            ...
            <div class="card mb-30" style="display: block;padding: 0px;">
                ...
                <div class="card-body" style="padding: 10px 25px;">
                    <div class="row journal-info">
                      ...
                      <div class="col-md-2">
                        <span class="text-justify no-margin">
                          <span class="sub-title">Declined By : </span>
                        </span>
                      </div>
                      <div class="col-md-10">
                          <span class="text-justify no-margin">
                    ...
                    <?php if(!empty($assigned_dates[$data->id])){ ?>
                      <div class="row">
                        <div class="col-md-2">
                          <span class="text-justify no-margin">
                            <span class="sub-title">Declined Date : </span>
                          </span>
                        </div>
                        <div class="col-md-10">
                            <span class="text-justify no-margin">
