...
$journal_data = $db->query("SELECT tj.id, tj.title, tj.author, tj.affiliation, tj.subject, tj.type, tj.year, tj.status, tj.manuscript_id, tj.resubmitted,tja.decline_other_reason,tdr.reason,tja.decline_reason FROM tbl_journal tj INNER JOIN tbl_journal_assigned tja ON (tj.id = tja.journal_id)INNER JOIN tbl_decline_reason tdr ON(tja.decline_reason = tdr.id) WHERE tj.status='6'");
    ...
    foreach($journal_data->rows as $index => $data){ ?>
      <div>
            <div class="card mb-30" style="display: block;padding: 0px;">
                    ...
                    <div class="row journal-info">
                      <div class="col-md-2">
                        <span class="text-justify no-margin">
                          <span class="sub-title">Reason : </span>
                        </span>
                      </div>
                      <div class="col-md-10">
                          <span class="text-justify no-margin">
                              <?php
                              $reason = '';
                              if($data->decline_reason == '4'){
                                $reason = $data->decline_other_reason;
                              } else {
                                $reason = $data->reason;
                              }
                              echo $reason;
                ...
                <div class="card-header d-flex justify-content-between align-items-center journal-title" style="margin-top : 0px; margin-bottom: 0px;padding : 5px 25px;">
                    <div class="row">
                      <div class="col-md-2">
                        <span class="text-justify no-margin">
